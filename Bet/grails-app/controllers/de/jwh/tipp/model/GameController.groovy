...
class GameController {
	...
	static allowedMethods = [save: "POST", update: "POST", delete: "POST",createBetFormInList:"GET"]
	...
	@Secured(['ROLE_ADMIN'])
	def save() {
		params.date =Date.parse("yyyy-MM-dd HH:mm", params.date)
		...
		flash.message = message(code: 'default.created.message', args: [
			message(code: 'game.label', default: 'Match'),
			gameInstance.id
		])
		...
		if (version != null) {
			if (gameInstance.version > version) {
				gameInstance.errors.rejectValue("version", "default.optimistic.locking.failure",
						[
							message(code: 'game.label', default: 'Match')] as Object[],
	...
	def createBetFormInList(Long id){
		def game = Game.get(id)
		def bet = Bet.findByGameAndUser(game,springSecurityService.currentUser)
		[bet:bet, game : game]
	}
}
